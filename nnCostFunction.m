...
%         computed in ex4.m
   ...
   a2 = [ones(m,1) a2];
   a3 = sigmoid(a2*transpose(Theta2));  % h
   identity = eye(num_labels);  
   y1_0 = zeros(m,num_labels);
   for i=1:m
       y1_0(i, :)= identity(y(i), :);  % to put only one non-zero value in column 
   end
   
   su = -y1_0.*log(a3) - (1-y1_0).*log(1-a3);
   J = (1/m)*(sum(sum(su,2)));  
   
   % with regularization %
    regul = sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2));
